...
%%%%%%%%%%% Camera Entry 1 %%%%%%%%%%%%%%%
    camera(1).name = "IMX708 Camera Module 3 Wide";
...
%%%%%%%%%%% Camera Entry 2 %%%%%%%%%%%%%%%
    camera(2).name = "IMX708 Camera Module 3 Narrow";
    ...
    for fb = flight_bounds
        ...
        for setting = 1:size(camera(cam_num).vid_res, 1)
            ...
            if plotFigs
                ...
                ylabel("Altitude (ft)");
                cb = colorbar;
                y1 = ylabel(cb,'Pixel Error (ft/pixel)','FontSize',10);
                ...
                dr = scatter3(0,0,fb, 50, 'k', 'filled');
                ...
                zlabel("Altitude (ft)");
                cb = colorbar;
                y1 = ylabel(cb,'Pixel Error (ft^2/pixel)','FontSize',12);
                yl.Position(1) = min(xlim(cb));
                yl.VerticalAlignment = 'bottom';
                clim(err_lim);
                colormap(c);
                legend(dr, "Drone Position");
                hold off;
            end
        end
    end
